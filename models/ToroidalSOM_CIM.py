...
def select_cim_sigma(data, subsample=1000, scale=1.0, 
    ...
    if scales_to_test is None:
        scales_to_test = [0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]
...
class ToroidalSOM_CIM(nn.Module):
    ...
    def __init__(self, grid_size, input_dim, batch_size, lr, lr_epoch_decay, cim_kernel_sigma, sigma,
                 sigma_epoch_decay, sigma_min, device, init_pca=False, data_for_init=None, **kwargs):
    ...
    def _initialize_weights(self):
        ...
        if self.init_pca and self.data_for_init is not None:
            ...
            unique_random_integers = np.random.choice(range(self.data_for_init.shape[0]), size=self.grid_size[0]*self.grid_size[1], replace=False)
    ...
    # def find_bmu(self, x):
    #     """Find Best Matching Units (BMUs) using CIM distance for each input vector."""
    ...
    def find_bmu(self, x, winner_penalty=0.0):
        distances = self.forward(x)  # Shape: (B, rows, cols)

        if winner_penalty > 0.0:
            # penalty = self.bmu_counts.float()
            # Normalize counts to [0, 1] range for stability
            # penalty = torch.pow(self.bmu_counts / (self.bmu_counts.max() + 1e-8), 2)
            penalty = self.bmu_counts / (self.bmu_counts.max() + 1e-8)
            penalty = penalty[None, :, :]  # Shape: (1, rows, cols)
            distances = distances + winner_penalty * penalty
    ...
    def update_weights(self, x, bmu_rows, bmu_cols):
        ...
        dist_sq = (col_diff + 0.5 * (row_diff % 2))**2 + (row_diff * 0.866)**2
        neighborhood = torch.exp(-dist_sq / (2 * self.sigma**2)) * neighborhood_mask  # (B, H, W)

        # Get BMU usage penalty for each sample
        bmu_indices = bmu_rows * cols + bmu_cols  # Shape: (B,)
        flat_counts = self.bmu_counts.view(-1).float()
        sample_penalties = 1.0 / (1.0 + flat_counts[bmu_indices])  # Shape: (B,)
        sample_penalties = sample_penalties.view(-1, 1, 1, 1)  # Broadcastable to (B, H, W, D)
        ...
        delta = self.lr * sample_penalties * neighborhood_expanded * (x_expanded - self.weights)

        # Apply update, averaging over batch
    ...
    def train(self, means_in, logvars_in, num_epochs, winner_penalty):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_start in range(0, num_samples, self.batch_size):
                ...
                # Find BMUs
                bmu_rows, bmu_cols = self.find_bmu(batch_sampled, winner_penalty)
