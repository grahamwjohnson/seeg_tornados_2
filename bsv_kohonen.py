    ...
    if (subset_override_dir != None) or (accumulated_data_pickle == None): # If override files or gathering raw files
        ...
        if (file_windowseconds != rewin_windowseconds) or (file_strideseconds != rewin_strideseconds):
            if (file_windowseconds > rewin_windowseconds) or (file_strideseconds > rewin_strideseconds):
                ...
                rewin_means_sentinel, rewin_logvars_sentinel, rewin_z_sentinel = utils_functions.rewindow_data(
                    ww_means_sentinel, ww_logvars_sentinel, w_z_sentinel, 
                ...
                print(f"Rewindowed shape of rewin_mogpreds: {rewin_z_sentinel.shape}")
                print(f"REDUCTION: {reduction}")
        else:
            rewin_means_sentinel = ww_means_sentinel
            rewin_logvars_sentinel = ww_logvars_sentinel
            rewin_z_sentinel = w_z_sentinel
        ...
        for i in range(len(data_filepaths)):
            ...
            try:
                ...
                ww_means = latent_data_fromfile['windowed_means']
                ww_logvars = latent_data_fromfile['windowed_logvars']
                w_z = latent_data_fromfile['windowed_z']
                if (file_windowseconds != rewin_windowseconds) or (file_strideseconds != rewin_strideseconds):
                    ww_means_allfiles[i, :, :], ww_logvars_allfiles[i, :, :], w_z_allfiles[i, :, :] = utils_functions.rewindow_data(
                        ww_means, ww_logvars, w_z, file_windowseconds, file_strideseconds, rewin_windowseconds, rewin_strideseconds, reduction=reduction)
        ...
        w_z_allfiles = data_loaded['w_z_allfiles']
